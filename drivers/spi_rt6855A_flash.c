...
extern  char led_update_blink;
extern void control_power_led(int flag);
...
struct chip_info *chip_prob(void)
{
	...
	for (i = 0; i < sizeof(chips_data)/sizeof(chips_data[0]); i++) {
		...
		if (info->id == buf[0]) {
#ifdef BBU_MODE
			printf("info id : %x info->jedec_id :%x buf[1]:%x buf[2]:%x\n", info->id, info->jedec_id, buf[1], buf[2]);
...
void flash_set_to_4byte(void)
{
	...
	raspi_write_enable();
	bbu_spic_trans(0xB7, 0, NULL, 1, 0, 0);
	raspi_write_disable();
	
	raspi_read_rg(0x15, &sr);
	printf("Before Read Status Register-3 : %x\n", sr);
	
	sr |= (1<<1);
	
	raspi_write_enable();
	raspi_write_rg(0x11, &sr);
		
	raspi_write_disable();
	raspi_read_rg(0x15, &sr);
	printf("After  Read Status Register-3 : %x\n", sr);
...
int do_mem_cp(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	} else if (!strncmp(argv[0], "cp.env", 9)) {
		CFG_ENV_ADDR - CFG_FLASH_BASE;
		printf("\n Copy uboot[%d byte] to SPI Flash[0x%08X].... \n", count, dest);
	} else {
		printf ("Usage:\n%s\n", cmdtp->usage);
		return 1;
...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	else if (strcmp(argv[1], "uboot") == 0) 
	{
		...
		printf("\n Erase env block !!\n");
		printf("From 0x%X length 0x%X\n", CFG_ENV_ADDR, CFG_CONFIG_SIZE);
		raspi_unprotect();
		rcode = raspi_erase(CFG_ENV_ADDR, CFG_CONFIG_SIZE);
		return rcode;
	}
...
int ralink_spi_command(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	if (!strncmp(argv[1], "id", 3)) {
		...
		}
		{
			...
			//读出整个64K 扇区数据
			//写地址偏移 offset对齐 64K
			read_len = raspi_read(read_p, o-(o%65536), 65536); //reuse len

			for (i = 0; i < read_len; i++) {
				//printf("%02x ", read_p[i]);
				cur_line[i%16] = read_p[i];
				#if 1
				if ( (i != 0) && ((i+1)%16 == 0) ) {
					if (memcmp(cur_line, last_line, 16) == 0) {
						continue;
					} else {
						#if 0
						printf("%08x\t", (i+1)-16);
						for (j = 0; j < 16; j++) {
							printf("%02x ", cur_line[j]);
						}
						printf("\n");
						#endif
						memcpy(last_line, cur_line, 16);
					}
				}
				#else
				if ( (i != 0) && ((i+1)%16 == 0) ) {
			...
			raspi_erase(o-(o%65536), 65536);	//先擦除

			for (i = 0; i < l; i++) {
				t[0] = argv[3][2*i];
				t[1] = argv[3][2*i+1];
				read_p[o%65536+i] = simple_strtoul(t, NULL, 16);
			}

			raspi_write(read_p, o-(o%65536), 65536);
			free(read_p);
		}
#if 0
		for (i = 0; i < l; i++) {
...
int ralink_spi_read_sr(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int val;
	
	raspi_read_rg(OPCODE_RDSR, &val);

	printf("Status Register 1 : %x\n", val);
	
	raspi_read_rg(0x35, &val);
	printf("Status Register 2 : %x\n", val);
	
	raspi_read_rg(0x15, &val);
	printf("Status Register 3 : %x\n", val);
}


int flash_switch_to_4byte(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	//进入4byte 模式
	raspi_4byte_mode(1);
...
int flash_exit_to_4byte(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	//退出4byte 模式
	raspi_4byte_mode(0);
...
int flash_always_to_4byte(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int retval;
	u8 sr;
...
int flash_always_to_3byte(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int retval;
	u8 sr;
	

	bbu_spic_trans(0xE9, 0, NULL, 1, 0, 0);
...
U_BOOT_CMD(
	spi_read_sr, CFG_MAXARGS, 1, ralink_spi_read_sr,
...
U_BOOT_CMD(
	flash_to_4byte, CFG_MAXARGS, 1, flash_switch_to_4byte,
	"flash_to_4byte switch to 4byte\n",
	"\n"
);

U_BOOT_CMD(
	flash_exit_4byte, CFG_MAXARGS, 1, flash_exit_to_4byte,
...
U_BOOT_CMD(
	flash_always_4byte, CFG_MAXARGS, 1, flash_always_to_4byte,
	"flash_always_4byte always switch to 4byte\n",
	"\n"
);

U_BOOT_CMD(
	flash_always_3byte, CFG_MAXARGS, 1, flash_always_to_3byte,
	"flash_always_3byte always switch to 3byte\n",
	"\n"
);
extern int burn_env(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);

U_BOOT_CMD(
	burn_env, CFG_MAXARGS, 1, burn_env,
	"burn_env\n",
	"\n"
);

extern int burn_uboot(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);

U_BOOT_CMD(
	burn_uboot, CFG_MAXARGS, 1, burn_uboot,
	"burn_uboot\n",
	"\n"
);

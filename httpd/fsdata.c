char data_404_html[] = {
/* HTTP Header */
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x34, 0x30, 0x34, 0x20, 0x46, 0x69, 0x6c, 0x65, 
0x20, 0x6e, 0x6f, 0x74, 0x20, 0x66, 0x6f, 0x75, 0x6e, 0x64, 0x0d, 0x0a,
0x53, 0x65, 0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x75, 0x49, 0x50, 0x2f, 0x30, 0x2e, 0x39, 0x0d, 0x0a,
0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x20, 0x74, 0x65, 0x78, 
0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x3b, 0x20, 0x63, 0x68, 0x61, 0x72, 0x73, 0x65, 0x74, 0x3d, 0x55, 
0x54, 0x46, 0x2d, 0x38, 0x0d, 0x0a, 0x0d, 0x0a,
/* Page/File content */
, 0 };

char data_art_html[] = {
/* HTTP Header */
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, 0x30, 0x30, 0x20, 0x4f, 0x4b, 0x0d, 0x0a,
0x53, 0x65, 0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x75, 0x49, 0x50, 0x2f, 0x30, 0x2e, 0x39, 0x0d, 0x0a,
0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x20, 0x74, 0x65, 0x78, 
0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x3b, 0x20, 0x63, 0x68, 0x61, 0x72, 0x73, 0x65, 0x74, 0x3d, 0x55, 
0x54, 0x46, 0x2d, 0x38, 0x0d, 0x0a, 0x0d, 0x0a,
/* Page/File content */
, 0 };

char data_fail_html[] = {
/* HTTP Header */
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, 0x30, 0x30, 0x20, 0x4f, 0x4b, 0x0d, 0x0a,
0x53, 0x65, 0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x75, 0x49, 0x50, 0x2f, 0x30, 0x2e, 0x39, 0x0d, 0x0a,
0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x20, 0x74, 0x65, 0x78, 
0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x3b, 0x20, 0x63, 0x68, 0x61, 0x72, 0x73, 0x65, 0x74, 0x3d, 0x55, 
0x54, 0x46, 0x2d, 0x38, 0x0d, 0x0a, 0x0d, 0x0a,
/* Page/File content */
, 0 };

char data_flashing_html[] = {
/* HTTP Header */
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, 0x30, 0x30, 0x20, 0x4f, 0x4b, 0x0d, 0x0a,
0x53, 0x65, 0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x75, 0x49, 0x50, 0x2f, 0x30, 0x2e, 0x39, 0x0d, 0x0a,
0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x20, 0x74, 0x65, 0x78, 
0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x3b, 0x20, 0x63, 0x68, 0x61, 0x72, 0x73, 0x65, 0x74, 0x3d, 0x55, 
0x54, 0x46, 0x2d, 0x38, 0x0d, 0x0a, 0x0d, 0x0a,
/* Page/File content */
, 0 };

char data_index_html[] = {
/* HTTP Header */
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, 0x30, 0x30, 0x20, 0x4f, 0x4b, 0x0d, 0x0a,
0x53, 0x65, 0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x75, 0x49, 0x50, 0x2f, 0x30, 0x2e, 0x39, 0x0d, 0x0a,
0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x20, 0x74, 0x65, 0x78, 
0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x3b, 0x20, 0x63, 0x68, 0x61, 0x72, 0x73, 0x65, 0x74, 0x3d, 0x55, 
0x54, 0x46, 0x2d, 0x38, 0x0d, 0x0a, 0x0d, 0x0a,
/* Page/File content */
, 0 };

char data_style_css[] = {
/* HTTP Header */
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, 0x30, 0x30, 0x20, 0x4f, 0x4b, 0x0d, 0x0a,
0x53, 0x65, 0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x75, 0x49, 0x50, 0x2f, 0x30, 0x2e, 0x39, 0x0d, 0x0a,
0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x20, 0x74, 0x65, 0x78, 
0x74, 0x2f, 0x63, 0x73, 0x73, 0x3b, 0x20, 0x63, 0x68, 0x61, 0x72, 0x73, 0x65, 0x74, 0x3d, 0x55, 0x54, 
0x46, 0x2d, 0x38, 0x0d, 0x0a, 0x0d, 0x0a,
/* Page/File content */
, 0 };

char data_uboot_html[] = {
/* HTTP Header */
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, 0x30, 0x30, 0x20, 0x4f, 0x4b, 0x0d, 0x0a,
0x53, 0x65, 0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x75, 0x49, 0x50, 0x2f, 0x30, 0x2e, 0x39, 0x0d, 0x0a,
0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d, 0x74, 0x79, 0x70, 0x65, 0x3a, 0x20, 0x74, 0x65, 0x78, 
0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x3b, 0x20, 0x63, 0x68, 0x61, 0x72, 0x73, 0x65, 0x74, 0x3d, 0x55, 
0x54, 0x46, 0x2d, 0x38, 0x0d, 0x0a, 0x0d, 0x0a,
/* Page/File content */
, 0 };

struct fsdata_file file_404_html;

struct fsdata_file file_art_html;

struct fsdata_file file_fail_html;

struct fsdata_file file_flashing_html;

struct fsdata_file file_index_html;

struct fsdata_file file_style_css;

struct fsdata_file file_uboot_html;

void fs_data_init(void)
{
	file_404_html.next = NULL;
	file_404_html.name = "/404.html";
	file_404_html.data = data_404_html;
	file_404_html.len = (int)sizeof(data_404_html) - 1;
	
	file_art_html.next = &file_404_html;
	file_art_html.name = "/art.html";
	file_art_html.data = data_art_html;
	file_art_html.len = (int)sizeof(data_art_html) - 1;
	
	file_fail_html.next = &file_art_html;
	file_fail_html.name = "/fail.html";
	file_fail_html.data = data_fail_html;
	file_fail_html.len = (int)sizeof(data_fail_html) - 1;
	
	file_flashing_html.next = &file_fail_html;
	file_flashing_html.name = "/flashing.html";
	file_flashing_html.data = data_flashing_html;
	file_flashing_html.len = (int)sizeof(data_flashing_html) - 1;
	
	file_index_html.next = &file_flashing_html;
	file_index_html.name = "/index.html";
	file_index_html.data = data_index_html;
	file_index_html.len = (int)sizeof(data_index_html) - 1;
	
	file_style_css.next = &file_index_html;
	file_style_css.name = "/style.css";
	file_style_css.data = data_style_css;
	file_style_css.len = (int)sizeof(data_style_css) - 1;
	
	file_uboot_html.next = &file_style_css;
	file_uboot_html.name = "/uboot.html";
	file_uboot_html.data = data_uboot_html;
	file_uboot_html.len = (int)sizeof(data_uboot_html) - 1;
}


#define FS_ROOT &file_uboot_html
#define FS_NUMFILES 7
